...
PostProcess <- function(x, cpts, delta, lambda, method){
  ...
  if (length(seg_boundaries) < 3)
    ...
    for (i in seq(2,length(seg_boundaries)-1)){
      ...
      opt_split_ind <- FindBestSplit(x[seg_inds,], delta = delta, n_obs = n_obs, optimizer = "line_search", control = NULL,
                                            SegmentLossFUN =  SegmentLoss(n_obs = n_obs, lambda = lambda, method = method))$opt_split
